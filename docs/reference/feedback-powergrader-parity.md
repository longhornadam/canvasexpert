...
The legacy FeedbackExpert page redirects to the PowerGrader migration lane. Its former
manual/run/push HTTP modules have been removed; manual Student Analysis CSV fallback,
legacy folder-result recovery, and persona/pattern administration remain available through
the PowerGrader replacement and shared compatibility readers.

## Current capability inventory

| Workflow | Current FeedbackExpert owner | Current PowerGrader owner | Parity / Luna 6 action |
|---|---|---|---|
| Assignment-scoped preparation | Retired `feedback_run.py` route; shared engines remain read-compatible | `routes/powergrader.py::pg_start` plus `powergrader/ai_workflow.py` | **Parity.** PG is canonical; it preserves a private session and focused evidence. |
| SAFE/PRIVATE artifact creation and safety scan | Retired HTTP run route; `feedback_artifacts.py`, `feedback_safety.py` remain shared engines | `powergrader/ai_workflow.py` reuses the same engines | **Parity.** Preserve honest pseudonymization wording and compatibility reads. |
| Own-AI-chat packet and pasted JSON | Retired HTTP push route; legacy folder data stays read-compatible | `packet.py`, `copilot_packet.py`, `import_results.py`, queue import UI | **Parity.** PG adds session/batch binding; it is the replacement. |
...
| OpenRouter drafting | Retired `feedback_run.py`/`feedback_manual.py` routes | `powergrader/ai_workflow.py` assisted mode after budget and safety gates | **Parity for ordinary assignment scoring.** PG is canonical; no provider abstraction change. |
| Teacher review and Assignment grade/comment push | Retired `feedback_push.py` direct preview/apply | PowerGrader review, preflight, push review/push, receipts | **Parity, stronger in PG.** Never weaken review, drift, or idempotency safeguards to imitate legacy routes. |
...
- Compatibility route: `api/webui/routes/feedback.py` (library API only) and
  `routes/pages.py::feedback_expert_page`; legacy manual/run/push route modules are removed.
