...
|---|---|---|---|---|---|---|---|
| 1 | **Home** – choose or continue work | `/` | `dashboard.html` (extends `workbench_base.html`) | `desk.js` | `routes/pages.py::dashboard`, `routes/work.py`, `routes/receipts.py`; explicit scan reductions in `work_registry/discovery.py` and `providers/home_attention.py` | `work-registry-contract.md`, `operation-ledger-release-status.md` | Exact generic jobs plus a transient local presentation sidecar; no Canvas calls from Home itself. Explicit scans reduce private submission/comment metadata to assignment-scoped counts only. |
...
|---|---|---|
| `base.html` | – | Settings, Welcome, Routines, AI Expert, About, Course Info |
| `workbench_base.html` | `base.html` | Home, Course Expert, Student Reports, Gradebook, Roster, PowerGrader (setup + queue) |

`workbench_base.html` adds `_workbench_header.html` (Home/Create/Grade/Students/Automations nav) and `readiness.js`. `base.html` has the original dropdown nav. Both share `style.css`, `workbench.css`, `write_review.js`, and `app_context.js`.
