...
| 9 | **Routines** | `/routines` | `routines.html` (`layouts/document.html`, `wide`) | inline / route-driven | `routes/routines.py` + `routes/routines_builtin.py` + `routes/routines_custom.py` + `routes/routines_powergrader.py` | `operation-ledger-contract.md` (routines integration) | Local automations only; no cloud scheduler; writes gated by routine definitions |
...
### August 2026: Flat packet ZIP download removed
