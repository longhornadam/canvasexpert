...
def test_mixed_scoring_keeps_text_group_and_isolates_media_failures(monkeypatch, tmp_path):
    calls = []
    ...
    class Vault:
        ...
        def reverse(self, pseudonym):
            uid = pseudo_to_uid.get(pseudonym)
            return {"canvas_id": uid, "real_name": "Fictional Student", "sis_id": ""} if uid else None
    ...
    bundle = {
        ...
        "students": [
            {"pseudonym": "P-T1", "responses": [{"item_id": "item-1", "response": "text"}]},
            {"pseudonym": "P-T2", "responses": [{"item_id": "item-2", "response": "text"}]},
            {"pseudonym": "P-M1", "responses": [{"item_id": "item-3", "response": "media", "media": [{"item_id": "item-3"}]}]},
            {"pseudonym": "P-M2", "responses": [{"item_id": "item-4", "response": "media", "media": [{"item_id": "item-4"}]}]},
        ],
    }
    safe_bundle = tmp_path / "safe.json"
    safe_bundle.write_text(json.dumps(bundle), encoding="utf-8")

    monkeypatch.setattr(ai_workflow.workspace, "ensure_workspace", lambda: None)
    monkeypatch.setattr(ai_workflow.context, "build_source_context", lambda *args, **kwargs: {"materials": []})
    monkeypatch.setattr(ai_workflow.context, "apply_shared_context", lambda current, *args: current)
    monkeypatch.setattr(ai_workflow.context, "vault", lambda: Vault())
    monkeypatch.setattr(ai_workflow.context, "load_rubric_text", lambda name: "synthetic rubric")
    monkeypatch.setattr(ai_workflow.config, "get_persona", lambda name: {"name": "Synthetic TA"})
    monkeypatch.setattr(ai_workflow.config, "list_feedback_patterns", lambda: [])
    monkeypatch.setattr(ai_workflow.config, "active_protected_names", lambda: set())
    monkeypatch.setattr(ai_workflow.config, "get_openrouter_key", lambda: "synthetic-key")
    monkeypatch.setattr(ai_workflow.source_materials, "context_warnings", lambda context: [])
    monkeypatch.setattr(ai_workflow.source_materials, "response_preset", lambda kind: {"output_tokens_per_student": 100})
    monkeypatch.setattr(ai_workflow.safety, "scan_payload", lambda payload, vault: {"green": True})
    monkeypatch.setattr(ai_workflow.fp, "pseudonymize_submissions", lambda *args: bundle)
    monkeypatch.setattr(ai_workflow.privacy, "feedback_artifact_dirs", lambda **kwargs: (str(tmp_path), str(tmp_path)))
    monkeypatch.setattr(ai_workflow.fp, "write_safe_and_private", lambda *args, **kwargs: {
        "safe_bundle": str(safe_bundle), "private_bundle": str(tmp_path / "private.json"),
        "who_is_who": str(tmp_path / "who.csv"), "how_to_score": str(tmp_path / "how.txt"),
        "student_txts": [], "attachment_only": [], "excluded": [], "shared_context": None,
        "shared_context_excluded": False,
    })
    monkeypatch.setattr(ai_workflow.packet, "build_safe_ai_packet", lambda *args, **kwargs: {})
    monkeypatch.setattr(ai_workflow.orc, "teacher_workflow_budget", lambda *args, **kwargs: {
        "ok": True, "estimated_cost": 0, "warnings": [], "reasons": [],
        "model_metadata": {"input_modalities": ["text", "image"]},
    })

    def score(one_bundle, *args, **kwargs):
        names = [student["pseudonym"] for student in one_bundle["students"]]
        calls.append(names)
        if names == ["P-M1"]:
            raise RuntimeError("provider detail stays out of session metadata")
        return [{"pseudonym": name, "item_id": student["responses"][0]["item_id"], "score": 8, "feedback": "Synthetic feedback"}
                for name, student in ((student["pseudonym"], student) for student in one_bundle["students"])]

    monkeypatch.setattr(ai_workflow.orc, "score", score)

    result = ai_workflow.run_ai_workflow(
        mode="assisted", submitted=[{"user_id": uid, "user": {"name": "Fictional Student"}, "body": "text"} for uid in pseudo_to_uid.values()],
        assignment_name="Fictional Essay", assignment_description="", course_id="course-1",
        course_name="Fictional Biology", assignment_id="assignment-1", session_id="session-1",
        rubric_name="", persona_id="sage", selected_model="synthetic-model", response_kind="scr",
