...
def test_pageforge_content_push_resolves_file_placeholder(monkeypatch, tmp_path):
...
""".strip(),
    ...
    def fake_canvas_get_all(path, params=None, timeout=30):
        assert path == "/api/v1/courses/42/files"
        assert params["search_term"] == "Handout.pdf"
        return [{
            "id": 99,
            "display_name": "Handout.pdf",
            "filename": "Handout.pdf",
            "url": "https://canvas.invalid/files/99/download",
        }], None

    monkeypatch.setattr(push_service, "_find_or_create_module_id", lambda cid, name, notes: None)
    monkeypatch.setattr(push_service, "_canvas_get_all", fake_canvas_get_all)
    monkeypatch.setattr(push_service, "_canvas_send", fake_canvas_send)
    ...
    assert sends[0][2]["wiki_page"]["body"] == (
        '<p><a href="https://canvas.invalid/files/99/download">Handout.pdf</a></p>'
    )


def test_pageforge_content_push_warns_on_unresolved_placeholder(monkeypatch, tmp_path):
    page_path = tmp_path / "placeholder.pageforge.json"
    page_path.write_text(
        """
<PAGEFORGE_JSON>
{"version":"1.0-json","type":"PAGE","title":"Handout","body":"<p>{{page:Missing Page}}</p>"}
</PAGEFORGE_JSON>
""".strip(),
        encoding="utf-8",
    )
    ...
    assert data["ok"] is True
    assert "{{page:Missing Page}}" in sends[0][2]["wiki_page"]["body"]
    assert "could not be resolved" in data["results"][0]["notes"][0]
