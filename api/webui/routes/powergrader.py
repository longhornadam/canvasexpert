...
def pg_start(
    ...
    result = start_workflow.run_start_session(
        ...
        oral_reading_enabled=oral_reading_enabled,
    ...
    if result["auto_post_enabled"] and mode == "assisted":
...
def pg_new_quiz_finalize(session_id: str, user_id: str = Form(""), review_token: str = Form(""), item_decisions: str = Form("")):
    ...
    if status_code == 200 and payload.get("status") == "finalized":
        session_actions.converge_new_quiz_after_finalize(
            _load_session(session_id), user_id, notify_write_through=_notify_write_through,
        )
    return JSONResponse(payload, status_code=status_code)
